...
multi-provider support, observability, async ingestion, and an eval harness
that actually caught real bugs before launch.

Built with **Next.js 15 · FastAPI · PostgreSQL + pgvector · Claude / OpenAI**.

> The flagship demo feature is **chat with your documents**: upload PDFs,
> watch them get chunked and embedded, then ask questions and get streamed
...
- **RAG pipeline** — async ingestion worker (parse → chunk with page
  numbers → embed → pgvector HNSW), agentic chat with a search tool,
  citations persisted with messages, per-tenant rate limiting (Redis,
  fail-open), structured-output metadata extraction.
...
flowchart LR
    subgraph web["apps/web — Next.js 15"]
        UI[App UI] --> BA[Better Auth<br/>sessions + JWT]
    end
    subgraph api["apps/api — FastAPI"]
        MW[JWT via JWKS<br/>+ X-Org-Id tenant scope] --> DOM[Domains<br/>chat · documents · usage · admin]
        DOM --> LLM[LLM layer<br/>providers · resilience · pricing]
        DOM --> RL[Rate limiter]
    end
    ...
    LLM --> ANT[Anthropic]
    LLM --> OAI[OpenAI]
    LLM --> VOY[Voyage embeddings]
    DOM --> PG[(Postgres 16<br/>+ pgvector)]
    ING --> PG
    RL --> RD[(Redis)]
    DOM -->|enqueue| RD
    RD -->|jobs| ING
...
- paste your **Anthropic or OpenAI key** in *Settings → AI Provider*
  (per-organization, encrypted at rest), or
...
Anthropic, or your OpenAI key covers both.
...
- **One normalized message format, many providers.** Domain code speaks one
  ...
  loop runs unchanged on Claude and GPT.
...
└── api/          # FastAPI — src/core (auth, tenancy, telemetry),
                  #           src/llm (providers, resilience, pricing),
                  #           src/domains (chat, documents, usage, admin, ...)
evals/            # eval harness, fixture corpus, per-commit results
infra/terraform/  # $0-tier deploy (in progress)
```
...
Actively developed. Remaining before v1: $0-tier deployment (Cloud Run free
tier + Neon + Upstash) and a public demo. Follow the ticket-by-ticket
history in [`tech-steps.md`](tech-steps.md).
